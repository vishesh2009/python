...
#def last_num:
#    if "+" in expression:
#        for myname in expression:
#            if myname != "+":
#                word_list.append(myname)
#            elif myname=="+" 
def bt_clear():
    global expression
    expression = ""
...
#         else:
#             expression=str()
#             expression=expression + str(multiply) + str(pisymbol)
#             input_text.set(expression)
#       
#         input_text.set(expression)
def delete():
...
pie = Button(btns_frame, text = "π", fg = "black", width = 30, height = 3, bd = 0, bg = "red", cursor = "hand2", command = pie,font=('Helvetica', 8, 'bold','italic'), activebackground="White").grid(row = 6, column = 0, padx = 1, pady = 1,sticky=W+E,columnspan=3) 
# fourth row
#One 
